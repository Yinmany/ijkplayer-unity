...
- (id) initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _overlayDatas = [[NSMutableArray alloc] init];
...
-(void) setupScaler:(SDL_VoutOverlay*) overlay
{
    ...
    imgConvertCtx = sws_getContext(overlay->w, overlay->h, AV_PIX_FMT_YUV420P, overlay->w, overlay->h, AV_PIX_FMT_RGB24, sws_flags, NULL, NULL, NULL);
...
-(void)convertFrameToRGB:(SDL_VoutOverlay*) overlay
{
    ...
    sws_scale(imgConvertCtx, (const uint8_t**)overlay->pixels, linesize, 0, overlay->h, picture.data, picture.linesize);
}
...
- (void)dealloc
{
    ...
    if(_unityExternalTexture){
        glDeleteTextures(1, &_unityExternalTexture);
        _unityExternalTexture = 0;
    }
...
- (GLuint) getUnityExternalTexture
{
    if(!_unityExternalTexture)
    {

        // 初始化Unity使用的纹理
        glGenTextures(1, &_unityExternalTexture);
        NSLog(@"初始化UnityExternal纹理:%i",_unityExternalTexture);
        glBindTexture(GL_TEXTURE_2D, _unityExternalTexture);
